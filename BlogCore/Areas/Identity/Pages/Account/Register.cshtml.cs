    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //AQUI VALIDAMOS SI LOS ROLES EXITEN
                    if (!await _rolManager.RoleExistsAsync(CNT.Administrador))
                    {
                        await _rolManager.CreateAsync(new IdentityRole(CNT.Administrador));
                    }

                    if (!await _rolManager.RoleExistsAsync(CNT.Registrado))
                    {
                        await _rolManager.CreateAsync(new IdentityRole(CNT.Registrado));
                    }

                    if (!await _rolManager.RoleExistsAsync(CNT.Cliente))
                    {
                        await _rolManager.CreateAsync(new IdentityRole(CNT.Cliente));
                    }
